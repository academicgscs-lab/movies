...
public class XRentalLoader implements Loadable<Rental>, Marshallable {
    private final XmlFacade<XRental> helper;
    ...
    public XRentalLoader(CustomerManager customerManager, MovieManager movieManager) {
        ...
        helper = new XmlFacade<>(XRental.class);
    }
    ...
    public boolean store() {
        boolean result = true;
        for (Customer customer : customerManager.getItems().values()) {
            for (Rental item : customer.getRentals()) {
                try {
                    helper.marshall(XRental.mapToXRental(item), FileManager.createFile(XRental.HOME, item.getId()));
                } catch (JAXBException e) {
                    System.out.println(e.getMessage());
                    result = false;
                }
